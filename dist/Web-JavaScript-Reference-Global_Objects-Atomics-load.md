...
<dd>The position in the `typedArray` to load from`.`</dd>
...
<small lang="en-US">The definition of 'Atomics.load' in that specification.</small>](http://tc39.github.io/ecmascript_sharedmem/shmem.html#Atomics.load)</td>
...
*   [`Atomics.store()`](/en-US/docs/Web/JavaScript/Reference/Global_Objects/Atomics/store "The static Atomics.store() method stores a given value at the given position in the array and returns that value. This atomic operation guarantees that no other write happens until the modified value is written back.")