`**decodeURIComponent()**` 方法用于解码由 [`encodeURIComponent`](/zh-CN/docs/Web/JavaScript/Reference/Global_Objects/encodeURIComponent "encodeURIComponent()是对统一资源标识符（URI）的组成部分进行编码的方法。它使用一到四个转义序列来表示字符串中的每个字符的UTF-8编码（只有由两个Unicode代理区字符组成的字符才用四个转义字符编码）。") 方法或者其它类似方法编码的部分统一资源标识符（URI）。
...
<small lang="zh-CN">decodeURIComponent</small>](http://www.ecma-international.org/ecma-262/5.1/#sec-15.1.3.2)</td>
...
<small lang="zh-CN">decodeURIComponent</small>](http://www.ecma-international.org/ecma-262/6.0/#sec-decodeuricomponent-encodeduricomponent)</td>
...
<small lang="zh-CN">decodeURIComponent</small>](https://tc39.github.io/ecma262/#sec-decodeuricomponent-encodeduricomponent)</td>
